...
class EspacioAcademico extends funcionGeneral{
   ...
   function __construct($configuracion)
   {
     ...
     $this->tema=$tema;
     $this->sql=new sql_espacio();
     
   }

}#Cierre de class EspacioAcademico
...
function procesar_componente($form_entrada)
{
	...
	$registro[4]=time();
	$registro[5]="Se registra un TIPO_VARIABLE de espacio academico, con nombre ".$registro[3];
	$log_us->log_usuario($registro,$configuracion);
...
function consultarAplicacionAcademica($cod_espacio)
    {
    ...
    //rescata el valor de la configuracion
     ...
     $obj_espacio=new EspacioAcademico($configuracion);
     $sentencia=$obj_espacio->sql->cadena_sql($configuracion,"espacioAplicacionAcademica",$cod_espacio);

     $acceso_db=$obj_espacio->conectarDB($configuracion,"oracle2");
     $resultado=$acceso_db->ejecutarAcceso($sentencia,"busqueda");
     

     if(is_array($resultado)){
 
        $cadena_html="<table border=0 class='bloquelateral' width='70%' align='center' cellpadding='2'"; 
        $cadena_html.="cellpacing='2' >";
	
        $cadena_html.="<tr class='bloquecentralcuerpo'>";
        $cadena_html.="<td colspan='2' align='center'  >";
        $cadena_html.="<strong>Información registrada en Aplicación Académica</strong>";
        $cadena_html.="</td><td><img src='<?echo $configuracion['site'].$configuracion['grafico']?>/cancelar.gif' align='rigth'   onclick='muestra_capa(\"aplicacionAcademica\",\"\")' valing='top' ></td></tr>";
        $cadena_html.="<tr>";
        $cadena_html.="<td  align='center'></td></tr>";
	$cadena_html.="<tr class='bloquecentralcuerpo'><td><strong>Codigo:</strong></td>";
        $cadena_html.="<td colspan='2'>".$resultado[0][2]."</td></tr>";
	$cadena_html.="<tr class='bloquecentralcuerpo'><td><strong>Nombre:</strong> </td><td >".ucwords(strtolower($resultado[0][3]))."</td></tr>";

	$cadena_html.="<tr class='bloquecentralcuerpo'><td colspan='2' >";
        $cadena_html.="<strong><strong>Asociado a: </strong></td></tr>";
        $cadena_html.="<tr>";
        $cadena_html.="<td colspan='2'>";
        $cadena_html.="<table border='0' align='left' class='bloquecentralcuerpo'>";
	for($i=0; $i<count($resultado); $i++)
        {
           $cadena_html.="<tr><td>";
           $cadena_html.="<img src='<?echo $configuracion['site'].$configuracion['grafico']?>/chk.png' border='0' >";
           $cadena_html.="</td><td>".ucwords(strtolower($resultado[$i][1]))."</td></tr>";
        }
        $cadena_html.="</table>";
        $cadena_html.="</td>";
        $cadena_html.="</tr>";
	$cadena_html.="</table>";
     ...
     }#Cierre de count resultado
     else{
        $cadena_html="<table border=0 class='bloquelateral'>";
       
        $cadena_html.="<tr class='bloquecentralcuerpo'><td><strong>Información registrada en Aplicación Académica</strong></td><td><img src='<?echo $configuracion['site'].$configuracion['grafico']?>/cancelar.gif'  onclick='muestra_capa(\"aplicacionAcademica\",\"\")' ></td><tr>";
        $cadena_html.="<tr class='bloquecentralcuerpo'><td colspan='2'>";
        $cadena_html.="Este espacio académico no se encuentra registrado en la aplicación";  $cadena_html.=" académica</td><tr>";
        $cadena_html.="</table>";
     }     

     $respuesta = new xajaxResponse();
     $respuesta->addAssign("aplicacionAcademica","innerHTML",$cadena_html);
     return $respuesta; 
