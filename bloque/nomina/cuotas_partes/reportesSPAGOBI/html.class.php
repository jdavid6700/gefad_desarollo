...
class html_estadoCuenta {
    ...
    function reporteContraloria($datos) {
    	...
    	$i = 0;
    	$parametros = explode("&", $datos[0]['parametro']);
    	$parametro = array();
    	
    	while($i < count($parametros)-1){
    		$i++;
    		$value = explode("=", $parametros[$i]);
    		
    		$parametro[$value[0]] = $value[1];
    	}
    	
    	$reporte = $parametro['nameReporte'];
    	$_REQUEST['usrReporte'] = $parametro['usrReporte'];
    	$_REQUEST['passReporte'] = $parametro['passReporte'];
    	$_REQUEST['roleReporte'] = $parametro['roleReporte'];
    	...
    	?>
    	        <link	href="<? echo $this->configuracion["host"] . $this->configuracion["site"] . $this->configuracion["bloques"] ?>/nomina/cuotas_partes/reportesSPAGOBI/form_estilo.css"	rel="stylesheet" type="text/css" />
    	        <!-- permite la paginacion-->        
    	
    	
    	        <h1>Reporte Cuotas Partes por Cobrar SPAGOBI</h1>
    	    	...
    	    	<html>
		            <head>
		                ...
		                <script type="text/javascript" src="bloque/nomina/cuotas_partes/reportesSPAGOBI/sbisdk-all-production.js"></script>
		            ...
		            <body>
		                	...
		                	<iframe id="execiframe" src='' height="80%" width="95%"></iframe>
