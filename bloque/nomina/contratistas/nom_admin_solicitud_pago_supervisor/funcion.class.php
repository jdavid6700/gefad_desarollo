...
__________________________________________________________________________________________________*/
   ...
   function consultarDatosRegistroPresupuestal($nro_cdp, $cod_unidad_ejecutora, $vigencia){
    ...
    function revisarCumplidosAprobados(){
        ...
        if(is_array($solicitudes)){
                foreach ($solicitudes as $key => $solicitud) {
                    ...
                    $datos_contratista = $this->consultarDatosContratista($num_id_contratista);
                    $datos_contrato = $this->consultarDatosContrato($cto_interno_co, $cum_cto_vigencia);
                    $id_concepto_nomina = $this->obtenerConceptoNomina($solicitud['num_contrato'],$cum_cto_vigencia);
                    ...
                    $registro[$key]['rubro_interno'] = $datos_disponibilidad[0]['RUBRO_INTERNO'];
                    ...
                    $registro[$key]['id_tipo_nomina'] = $id_concepto_nomina;
    ...
    function revisarSolicitudPagoNomina(){
        ...
        if($total){
                ...
                for($i=0;$i<$total;$i++){
                    ...
                    $nombre_tipo_nomina= "id_tipo_nomina_".$i;
                    ...
                    if($_REQUEST[$nombre]){
                            ...
                            if(!$existe){
                                    ...
                                    $registro[$indice]['id_tipo_nomina']=$_REQUEST[$nombre_tipo_nomina];
                                    $indice++;
                            }
                    }
        ...
        if($registro){
                ...
                $rubros = $this->obtenerRubroYConceptoSolicitudes($registro);
                ...
                foreach ($datos_solicitudes as $key => $datos_solicitud) {
                    ...
                    $id_tipo_nomina= $datos_solicitud['id_tipo_nomina'];
                    $anio=$anio_pago;
                    $mes=$mes_pago;
                    ...
                    $insertado = $this->insertarSolicitudPago($id_solicitud_pago,$rubro_interno,$anio, $mes, $cod_supervisor, $id_supervisor, $cod_ordenador, $id_ordenador,$cod_dependencia,$id_tipo_nomina);
     ...
     * @return type 
     */
    function obtenerRubroYConceptoSolicitudes($registros){
        $indice=0;
        $rubro= array();
        if($registros){
                foreach ($registros as $key => $registro) {
                    $existe = $this->buscarRubroInternoEnArreglo($rubro,$registro['rubro_interno'],$registro['id_tipo_nomina']);
                    if($existe==0){
     ...
     * @return int 
     */
    function buscarRubroInternoEnArreglo($rubros,$rubro_interno,$tipo_nomina){
        $existe =0;
        foreach ($rubros as $rubro) {
            if($rubro['rubro_interno']==$rubro_interno && $rubro['id_tipo_nomina']==$tipo_nomina){
                $existe=1;
            }
        }
        return $existe;
     ...
     * @return int 
     */
    function insertarSolicitudPago($id_solicitud_pago,$rubro_interno,$anio, $mes, $cod_supervisor, $id_supervisor, $cod_ordenador, $id_ordenador,$cod_dependencia,$id_tipo_nomina){
        ...
        if($datos_solicitudes && $registros){
            foreach ($datos_solicitudes as $key => $datos_solicitud) {
                foreach ($registros as $registro) {
                    if($datos_solicitud['rubro_interno']==$registro['rubro_interno'] && $datos_solicitud['id_tipo_nomina']==$registro['id_tipo_nomina']){
...
/**
    ...
    /**
     * Funcion para obtener el codigo del concepto de nomina relacionado a un contrato
     * @param int $num_contrato
     * @param int $vigencia_contrato
     * @return string 
     */
    function obtenerConceptoNomina($num_contrato,$vigencia_contrato){
        $concepto = $this->consultarActaInicio($num_contrato, $vigencia_contrato);
        return $concepto[0]['aci_cno_codigo'];
        
    }
    
    
    /**
     * Funcion para consultar los datos que se encuentran registrados en el acta de inicio
     * @param type $num_contrato
     * @param type $vigencia_contrato
     * @return type 
     */
    function consultarActaInicio($num_contrato,$vigencia_contrato){
        
            $datos = array( 'cod_contrato'=>$num_contrato,
                            'vigencia_contrato'=>$vigencia_contrato
                            );
