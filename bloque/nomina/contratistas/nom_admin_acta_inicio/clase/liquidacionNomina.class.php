...
class liquidacionNomina  extends funcionGeneral{
    ...
    function consultarCuentasDescuentos(){
        
        $cadena_sql=$this->cadena_sql("cuentas_descuentos",'');
        //echo "<br>cadena ".$cadena_sql;
        return $resultado=$this->ejecutarSQL($this->configuracion, $this->acceso_nomina, $cadena_sql, "busqueda");
    
    }
    
    function buscarCuentasDescuentos($cuentas_descuentos, $cod_descuento,$tipo_cuenta){
        
        foreach ($cuentas_descuentos as $key => $descuento) {
            if ($descuento['des_id'] == $cod_descuento){
                $nombre_cuenta = "des_cta_".strtolower($tipo_cuenta);
                $codigo_cta = $descuento[$nombre_cuenta];
                break;
            }
        }
        return $codigo_cta;
		...
		switch($opcion)
		{
			...
			case "variables_liquidacion":
			 ...
			 case "salario_base":
                        ...
                        case "cuentas_descuentos":
                                $cadena_sql=" SELECT des_id,";
                                $cadena_sql.=" des_nombre,";
                                $cadena_sql.=" des_descripcion,";
                                $cadena_sql.=" des_estado,";
                                $cadena_sql.=" des_tpd_id,";
                                $cadena_sql.=" des_cta_debito,";
                                $cadena_sql.=" des_cta_credito";
                                $cadena_sql.=" FROM fn_nom_descuento";
                                $cadena_sql.=" WHERE des_tpd_id=2";
                                break;
