...
function seleccion_valida(formulario,control)
{
   ...
   // var seleccionado = document.getElementById(control).checked;
...
function opcion_valida(formulario, control)
{
    var check=0;
    for (var i=0; i<formulario.elements.length; i++){
        var elemento = formulario.elements[i];
        if(elemento.type == control) {
            if(elemento.checked) {
                check=1;
            }
        }
...
} // Fin de la función opcion_valida

function checkbox_seleccionado(formulario, control)
{
    var seleccionado = document.getElementById(formulario);
    if (!seleccionado)
	{
            alert('Debe seleccionar una opción.');
            campo.focus();
...
/*********************************************************************************************************
*Funcion para adicionar filas a la tabla de usuarios
*Esta funcion recibe, el formulario, 
*control1=tbody donde se van a adicionar las filas
*control2=select con la carrera o item que se va a adicionar
*img=consecutivo que identifica el tab donde se adiciona la fila
*string=nombre de la tabla, esto para formar el llamado a la funcion delCarrera
**********************************************************************************************************/

function addCarrera(formulario, control1, control2, img, string){
   // alert ("seuimiento"+img);
    var indice=control2.selectedIndex;
    var nom=control2.options[indice].text;
    var idCra=control2.options[indice].value;
    var fila = document.createElement("tr");
    var celda1 = document.createElement("td");
    var celda2 = document.createElement("td");
    var p=document.getElementById("img").value;
    var consecutivo;
    ...
    for (var i=1; i<datos; i++){
     ...
     consecutivo=cadena[0];
     
    }else {
        consecutivo=1;
    }
    
    var nombre="consecutivo"+img+"_1";
    document.getElementById(nombre).value=consecutivo;

    if (bandera != 1 || datos==1){
    
    celda1.innerHTML = "<input type='text' value='"+nom+"' readonly='readonly' name ='textcarrera"+((control1.getElementsByTagName("tr").length))+"' size='45'>\n\
    <input type='hidden' name='control"+img+consecutivo+"' value='"+idCra+"' id='carrera_"+((control1.getElementsByTagName("tr").length))+"_"+idCra+"'>";
    celda2.innerHTML = "<input type='image' src='"+p+"' onclick='delCarrera("+string+","+((control1.getElementsByTagName("tr").length))+")'><input type='hidden' name='valor"+img+"' value='"+(control1.getElementsByTagName("tr").length+1)+"'>";

    fila.appendChild(celda1);
    fila.appendChild(celda2);
    control1.appendChild(fila);
...
function addModulo(formulario, control1, select_control2, select_control3, string){
    ...
    document.getElementById(nombre).value=datos;
     ...
     if (datos==0){
        ...
        <input type='hidden' name='idTar"+((datos)+1)+"' value='"+idnom2+"' id='idTar"+((datos)+1)+"'>\n\
        ...
        <input type='hidden' name='idTar"+((datos))+"' value='"+idnom2+"' id='idTar"+(datos)+"'>\n\
...
/*********************************************************************************************************
*Funcion para eliminar filas en una tabla
*control1=tabla, tbody
*fila=numero de fila que se quiere eliminar
**********************************************************************************************************/
