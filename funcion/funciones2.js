...
function setPointer(theRow, theRowNum, theAction, theDefaultColor, thePointerColor, theMarkColor)
{
    ...
    else if (currentColor.toLowerCase() == theMarkColor.toLowerCase()) 
    {
        if (theAction == 'click') 
	{
            newColor= (thePointerColor != '')? thePointerColor:theD oncontextmenu='return false;'efaultColor;
...
function longitud_cadena(formulario,control,tamanno)
{
    ...
    if (longitud<tamanno) {
        campo.select();
	alert1('Dato incorrecto. El campo debe contener un dato con mínimo '+tamanno+' caracteres');
        campo.focus();
	return false;
    } 
...
function solo_texto_sin_esp(evt){
		...
		if ((charCode < 91 && charCode > 64)  || (charCode < 123 && charCode > 96) ||  (charCode < 166 && charCode > 159) ||  charCode==8  ||  charCode==32){
			if(evt.ctrlKey) 
				return false;
			else
				return true;
...
function verificar_telefono(formulario,control) {
   	...
   	if (num <= 1111111){
		campo.select();
		alert1('Número telefónico incorrecto.');
		campo.focus();
		return false;
    	}	 
	return true;
