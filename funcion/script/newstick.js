...
function NewsTicker(x,y,width,height,color) {
	this.name = 'NewsTicker'+(NewsTicker.count++)
	this.x = x
	this.y = y
	this.w = width
	this.h = height
	this.color = (arguments.length>=5 && color!=null)? color : null;//"#FFFFFF";
	this.obj = this.name + "Object"
	eval(this.obj + "=this")
	this.items = new Array()
	this.scrollCount = 0

	this.pauseLength = 9000	//Pausa en milisegundos
	this.inc = 2
	this.speed = 20 //Velocidad de desplazamiento
	this.fromX = 0
	this.fromY = this.h
...
function NewsTickerBuild() {
	this.css = css(this.name,this.x,this.y,this.w,this.h,this.color)
